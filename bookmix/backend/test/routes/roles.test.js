...
describe('roles routes', () => {
	...
	it('POST / -> 201', async () => {
		pool.query.mockResolvedValueOnce({ rows: [{}] });
		const res = await request(app).post('/').send({ rolename: 'user' });
		expect(res.status).toBe(201);
	});
	it('PUT /:id -> 200 and 404', async () => {
		pool.query.mockResolvedValueOnce({ rows: [{}] });
		let res = await request(app).put('/1').send({ rolename: 'manager' });
		expect(res.status).toBe(200);
		pool.query.mockResolvedValueOnce({ rows: [] });
		res = await request(app).put('/999').send({ rolename: 'manager' });
		expect(res.status).toBe(404);
	});
